...
void defineTests(FileSystemTestContext ctx) {
  ...
  group('pubtestdependencies', () {
    ...
    test('simple_dependencies', () async {
      ...
      IoFsPubPackage exampleSimpleDependencyPkg =
      new IoFsPubPackage(childDirectory(pkgDir, join('example', 'simple_dependency')));
      ...
      IoFsPubPackage pkgDependency = await exampleSimpleDependencyPkg.clone(dstDependency);
      await pkg.runPub(pubGetArgs(offline: true), connectStderr: true, connectStdout: false);
      ProcessResult result =
          await pkg.devRunCmd(dartCmd([pubTestDependenciesDartScript, '--get-offline', '-r', 'json'])
            // p', 'vm'])
            ..connectStderr = true
            ..connectStdout = false);
    ...
    solo_test('simple_failed_dependencies', () async {
      Directory top = await ctx.prepare();
      IoFsPubPackage exampleSimplePkg =
      new IoFsPubPackage(childDirectory(pkgDir, join('example', 'simple_failed')));
      IoFsPubPackage exampleSimpleDependencyPkg =
      new IoFsPubPackage(childDirectory(pkgDir, join('example', 'simple_failed_dependency')));
